...
sub first {
    my $self = shift;
    return (ref $self || $self)->new_from_element(Array->new($self->{trees}->get(0) || ()), $self);
}
 
sub last {
    my $self = shift;
    return (ref $self || $self)->new_from_element(Array->new($self->{trees}->get(-1) || ()), $self);
}
...
sub find {
    ...
    $self->{trees}->each(sub {
        my $tree = shift;
        $new->push($tree) if defined $tree->parent && $tree->matches($xpath_rootless);
        $new->push($tree->findnodes(selector_to_xpath($selector, root => defined $tree->parent ? './' : '/')));
    });
